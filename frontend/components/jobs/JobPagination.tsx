...
import type { JobPageMetadata } from "@/types/jobs";

export default function JobPagination({ metadata, loading, onPage }: {
  metadata: JobPageMetadata | null; loading: boolean; onPage: (page: number) => void;
}) {
  if (!metadata || metadata.totalPages <= 1) return null;
  return (
    <nav aria-label="Jobs pagination" className="mt-5 flex items-center justify-between">
